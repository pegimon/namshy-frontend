import { ProductionQuantityLimits } from "@mui/icons-material";

export const size=[1,2,3,4,4,5,6,6,1,2,2,3,4,4,5,5,6,6,7,7,8,8,9,9,9,7,2,6,6,1,5,5,4,4,4,4311]
export const type=["page 2","link","link1","link","link1","link"]
export const options=["option1","option2","option3","option4","option1","option2","option3","option4","option5","option6"]
export const colors=[
{id:1,
name:"red",
value:"danger"
},
{id:2,
name:"green",
value:"success"
},
{id:3,
name:"black",
value:"dark"
},
{id:4,
name:"baby blue",
value:"info"
},
{id:5,
name:"yellow",
value:"warning"
},
{id:6,
name:"gray",
value:"secondary"
},
{id:7,
name:"blue",
value:"primary"
},
{id:8,
name:"wihte",
value:"light"
},
]
export const products = [
  {
    id: 1,
    images: [
      
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "9334 USK",
    old:"4000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 2,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 3,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 4,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 5,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 6,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 7,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 8,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 9,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  },
  {
    id: 10,
    images: [
      
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "9334 USK",
    old:"4000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 11,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 12,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 13,
    images: [
      
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "9334 USK",
    old:"4000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 14,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  }, {
    id: 15,
    images: [
      {
        num: 8,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 7,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 1,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "First slide",
      },
      {
        num: 2,
        src:
          "https://images.pexels.com/photos/9218538/pexels-photo-9218538.jpeg?auto=compress&cs=tinysrgb&w=600",
        alt: "Second slide",
      },
      {
        num: 3,
        src:
          "https://images.pexels.com/photos/712316/pexels-photo-712316.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
        alt: "Third slide",
      },
    ],
    productname: "Faux Leather Skirt",
    productdescriptiontitle: " Casual with edgy edits, this skirt adds up for an effortlessly comfortable fit. ",
    productdescription: ["Smooth and comfortable faux leather fabric",
      "Concealed zipper closure to waistband"
     ," Press stud front placket for versatile styling options"
     ," Mini length A-line fit"
      ,"Embroidered adidas Originals signature branding"
     , "Each item sold separately"],
    price: "4334 USK",
    old:"9000 USK",
    color: "red",
    brand: "adidas Originals",
    arrivals: "blabla",
    specialOffers: true,
    occasions: "blabla",
    style: "blabla",
    size: ["m", "l", "xl", "xxl"],
    sku:"	14448ATNCRXP",
    ModelHeight:"173cm",
    NeackType:"	Collar Neck",
    appareltype:"	Skirt",
    sizeShown:"m",
    supplier:"	II6100",
    washing:"	Wash according to instructions on care label",
    Product:"	100% PU Faux Leather",
    barandicone:"11111",
    Measurements:"	Bust: 83 cm - Waist: 62 cm - Hips: 92 cm",
  },
];
